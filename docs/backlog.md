...
| PBI-S0-008 | Stage 0 | Open | PBI-S0-006, PBI-S0-007 | Run a measured real-checkpoint full-layer ciphertext handoff chain at an agreed tiny size with no intermediate decrypts, validated max-error JSON, operation counts, and artifact validation. Attempts `10151`/`10153`/`10154` and `10164`-`10169` show depth exhaustion, HE-standard ring constraints, and 2-layer timeouts; `10170` proves one full visible OpenFHE layer can pass (`432.44s` wall clock, `max_abs_error=2.14e-02`) but does not provide inter-layer handoff. New partial-visible evidence: full inferred shape is blocked by rotation inventory (`10195`, `23498` keys / `11749.0 GiB`) and unstable in tracking (`10193`, `max_abs_error=3.45e111`); adapted shape passes tracking for 2 layers (`10196`, `max_abs_error=6.53e-04`) and OpenFHE for 1 layer (`10198`, `max_abs_error=5.85e-04`), while 2-layer OpenFHE depth-28 still exhausts depth in SiLU (`10197`). |
| PBI-S0-009 | Stage 0 | Open | PBI-S0-008 | Scale the measured full-layer ciphertext handoff chain to the scheduled 24-layer recurrence plan or document a smaller proxy when cost is prohibitive; include encode/encrypt/eval/bootstrap/decrypt profiler breakdown. Given `10168`/`10169` and `10195`, the likely next executable proxy is an adapted/packed partial-visible chain with lower-degree activations or higher depth/ring, then Stage 1 packing/sketching; another unoptimized full `d_model=768`, `mimo_rank=1536` OpenFHE two-layer run should be treated as a guard-only diagnostic. |
