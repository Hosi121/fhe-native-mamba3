...
This is the canonical product backlog for the current `0.2.x` line. It records
...
`0.3.0` should wait for PBI-S1-005: an integrated tiny encrypted MIMO/SSD block
smoke. Existing Stage 0 and Stage 1 pieces are useful prerequisites, but they do
not yet prove the combined packed SSD/prefix-scan path inside a tiny encrypted
block.
...
| PBI-S1-005 | Stage 1 | Open | PBI-S0-008, PBI-S1-004 | Integrate tiny encrypted MIMO/SSD block smoke: combine encrypted tiny MIMO recurrence/readout with packed SSD/prefix-scan layout metadata, run without unsupported full-chain claims, emit benchmark/error JSON, and add a focused script test. This is the likely `0.3.0` release gate. |
...
- README version `0.2.100` was stale and has been updated to `0.2.135`.
- Any backlog item phrased as "full OpenFHE chain success" is stale unless it
  ...
  and full-layer gate/proxy artifacts, segmented prefix-scan planning, and a
  Stage 1 prefix-scan smoke.
