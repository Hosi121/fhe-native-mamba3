...
| PBI-S2-001 | Stage 2 | Open | PBI-S1-005 | Empirically test sketch dimensions for MIMO SSM trajectories before claiming theory-driven dimension choices; synthetic SRHT sweep and checkpoint-source trace ingestion are implemented in `scripts/run_stage2_sketch_sweep.py`, `scripts/run_checkpoint_source_sketch_trace.py`, `tests/test_stage2_sketch_sweep.py`, and `tests/test_checkpoint_sketch_trace.py`; acceptance still requires recorded checkpoint-derived sweep JSON and error/latency tradeoff notes. |
...
- README version `0.2.100` was stale and has been updated through `0.3.4`.
