...
auto main(int argc, char* argv[]) -> int {
  try {
    ...
    for (int t = 0; t < config.seq_len; ++t) {
      ...
      for (int i = 0; i < state_slots; ++i) {
        h_expected[static_cast<size_t>(i)] =
            decays.back() * h_expected[static_cast<size_t>(i)] + b[static_cast<size_t>(i)] * x[static_cast<size_t>(i)];
      }
    }
    ...
    for (int t = 0; t < config.seq_len; ++t) {
      ...
      auto update = cc->EvalMult(encrypted_inputs[static_cast<size_t>(t)], b_plaintexts[static_cast<size_t>(t)]);
    ...
    std::cout << "\"ct_pt_mul\":" << (2 * config.seq_len + unity_multiplies) << ",";
    ...
    std::cout << ",\"next_bottleneck\":\"ct-pt multiplies and level alignment before packed readout/SSD scan\"";
    std::cout << "}" << std::endl;
  } catch (const std::exception& exc) {
