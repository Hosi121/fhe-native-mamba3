...
def main() -> int:
    ...
    rotations = _required_grouped_rotations(
        helpers=helpers,
        d_model=int(layer_input.shape[-1]),
        d_state=d_state,
        mimo_rank=mimo_rank,
...
def _required_grouped_rotations(
    *,
    helpers: Any,
    d_model: int,
    d_state: int,
    ...
    """Return a safe rotation-key superset for the grouped checkpoint gate.

    PBI-S1-014 still keeps pre-recurrence full-rank, then compacts per-rank
    slices for grouped recurrence/lift. The normal full-layer inventory covers
    the full-rank pre-recurrence path; this helper adds the grouped compaction,
    local readout, gate alignment, and local visible projection rotations.
