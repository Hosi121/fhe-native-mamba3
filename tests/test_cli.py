...
def test_weight_bundle_cli_exports_and_inspects_manifest(tmp_path) -> None:
    ...
    assert export_payload["version"] == __version__
    ...
    assert inspect_payload["version"] == __version__
...
def test_weight_bundle_generate_cli_runs_client_side_argmax(tmp_path) -> None:
    ...
    assert payload["client_decrypt_count"] == 3
    assert payload["output_payload_width"] == 16
    assert payload["prompt_token_ids"] == [1, 2, 3]
    assert len(payload["new_token_ids"]) == 3
    assert len(payload["generated_token_ids"]) == 6
    assert len(payload["decode_steps"]) == 3
    assert payload["decode_steps"][0]["output_payload_width"] == 16
...
def test_mamba_checkpoint_recurrence_smoke_cli_runs_tracking_backend(tmp_path) -> None:
    ...
    assert payload["ckks"]["bootstrap_after_tokens"] == [1]
    assert payload["operation_counts"]["bootstraps"] == 1
...
def test_mamba_checkpoint_recurrence_smoke_cli_runs_source_dynamic_bc(tmp_path) -> None:
    ...
    assert payload["depth_advisory"]["recommended_multiplicative_depth"] == 5
    assert payload["depth_advisory"]["has_recommended_depth"] is True
...
def test_mamba_checkpoint_full_layer_gate_cli_runs_tracking_backend(tmp_path) -> None:
    checkpoint_path = tmp_path / "mamba.pt"
    output_json = tmp_path / "full-layer-gate.json"
    torch.save({"model": _fake_mamba_state_dict()}, checkpoint_path)
    ...
    payload = json.loads(completed.stdout)
    assert payload["version"] == __version__
    assert payload["stage"] == "mamba-checkpoint-full-layer-gate"
    assert payload["backend"] == "tracking"
    assert payload["encrypted"] is False
    assert payload["model"]["seq_len"] == 2
    ...
    assert payload["measurement_scope"]["official_mamba_parity"] is False
    assert payload["measurement_scope"]["full_model_correctness_claimed"] is False
    assert payload["result"]["full_layer_formula_checked"] is True
    assert payload["result"]["visible_handoff_ciphertext"] is True
...
def test_mamba_checkpoint_full_layer_gate_cli_labels_partial_visible_output(
    ...
    assert payload["measurement_scope"]["source_style_full_layer_formula"] is False
    ...
    assert payload["result"]["full_layer_formula_checked"] is False
    assert payload["result"]["partial_visible_output_checked"] is True
    assert payload["passed"] is True


def test_mamba_checkpoint_full_layer_gate_cli_rejects_openfhe_rotation_explosion(
    tmp_path,
) -> None:
    checkpoint_path = tmp_path / "mamba.pt"
    torch.save({"model": _fake_mamba_state_dict()}, checkpoint_path)
    ...
    assert "rotation keys" in completed.stderr


def test_mamba_checkpoint_recurrence_smoke_cli_uses_scale_plan(tmp_path) -> None:
    checkpoint_path = tmp_path / "mamba.pt"
    bundle_dir = tmp_path / "mamba-scale-plan-bundle"
    scale_plan_path = tmp_path / "scale-plan.json"
    torch.save({"model": _fake_mamba_state_dict()}, checkpoint_path)
    ...
    assert payload["depth_advisory"]["recommended_multiplicative_depth"] == 5
    assert payload["model"]["scale_plan"]["cli_state_scale_override"] is False
    assert payload["model"]["scale_plan"]["cli_output_scale_override"] is False
    assert payload["max_abs_error"] < 1e-12


def test_mamba_checkpoint_recurrence_smoke_cli_rejects_low_openfhe_depth(
    tmp_path,
) -> None:
    checkpoint_path = tmp_path / "mamba.pt"
    bundle_dir = tmp_path / "mamba-low-depth-bundle"
    torch.save({"model": _fake_mamba_state_dict()}, checkpoint_path)
    ...
    assert completed.returncode != 0
    assert "below the recurrence depth estimate 5" in completed.stderr
...
def test_mamba_checkpoint_recurrence_sweep_cli_reports_rows(tmp_path) -> None:
    ...
    scale_plan_path = tmp_path / "sweep-scale-plan.json"
    torch.save({"model": _fake_mamba_state_dict()}, checkpoint_path)
    scale_plan_path.write_text(
        json.dumps(
            {
    ...
    assert payload["sweep_config"]["input_propagation"] == "source"
    assert payload["sweep_config"]["scale_plan_json"] == str(scale_plan_path)
    assert payload["sweep_config"]["ckks_max_level"] == 28
    assert payload["sweep_config"]["ckks_min_level"] == 2
    assert payload["summary"]["max_ct_ct_mul"] == 3 * 2
    assert payload["summary"]["bootstrap_schedules"]["group_count"] == 4
    assert payload["summary"]["bootstrap_schedules"]["max_bootstraps"] == 0
    assert payload["summary"]["bootstrap_schedules"]["groups"][0]["segments"]
    ...
    assert all(row["state_scale"] == 0.5 for row in payload["rows"])
    assert all(row["output_scale"] == 0.25 for row in payload["rows"])
    assert all(row["scale_plan"]["path"] == str(scale_plan_path) for row in payload["rows"])
    assert all(row["depth_advisory"]["has_recommended_depth"] is True for row in payload["rows"])
    assert dynamic_rows[-1]["operation_counts"]["ct_ct_mul"] == 3 * 2
    assert dynamic_rows[-1]["input_propagation"] == "source"
...
def test_source_diagnostics_scale_plan_cli_accepts_source_profile_payload(tmp_path) -> None:
    profile_json = tmp_path / "profile.json"
    output_json = tmp_path / "profile-scale-plan.json"
    ...
    payload = json.loads(completed.stdout)
    assert payload["version"] == __version__
    assert payload["scale_plan"]["layers"][0]["activation_scale_to_target"] == 0.5
    assert payload["scale_plan"]["layers"][0]["output_scale"] == 32.0 / 200.0
    assert json.loads(output_json.read_text(encoding="utf-8"))["scale_plan"]["layer_count"] == 1
...
def test_checkpoint_map_template_cli_writes_reusable_rules_json(tmp_path) -> None:
    ...
    assert template_payload["version"] == __version__
